...
export function agentProcessUrl() {
    const base = apiBase();
 ...
 */
export function parseAgentSseBody(raw) {
    let answer = "";
...
export async function postAgentProcess(input, options) {
    ...
    if (!res.ok) {
        ...
        throw new Error(t ? `HTTP ${res.status}: ${t.slice(0, 200)}` : `HTTP ${res.status}`);
    }
    const raw = await res.text();
    return parseAgentSseBody(raw);
}
function processSseLine(line, onDelta, accumulated) {
    const s = line.trim();
    if (!s.startsWith("data:"))
        return;
    const chunk = s.slice(5).trim();
 ...
 */
export async function streamAgentProcess(input, options) {
    const res = await fetch(agentProcessUrl(), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
