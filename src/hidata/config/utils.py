...
def get_available_channels() -> Tuple[str, ...]:
    ...
    hidata.channels), filtered by HIDATA_ENABLED_CHANNELS when set.
    """
    from ..app.channels.registry import get_channel_registry

    registry = get_channel_registry()
    all_keys = tuple(registry.keys())
    raw = os.environ.get("HIDATA_ENABLED_CHANNELS", "").strip()
    if not raw:
        return all_keys
...
def is_running_in_container() -> bool:
    ...
    Prefer env HIDATA_RUNNING_IN_CONTAINER (1/true/yes) at call time so
