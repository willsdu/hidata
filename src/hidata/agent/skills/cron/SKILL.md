---
name: cron
description: 通过 hidata 命令管理定时任务 - 创建、查询、暂停、恢复、删除任务
metadata: { "hidata": { "emoji": "⏰" } }
---

# 定时任务管理

使用 `hidata cron` 命令管理定时任务。
...
# 查看任务详情
hidata cron get <job_id>

# 查看任务状态
hidata cron state <job_id>

# 删除任务
hidata cron delete <job_id>

# 暂停/恢复任务
hidata cron pause <job_id>
hidata cron resume <job_id>

# 立即执行一次
hidata cron run <job_id>
```
...
```bash
hidata cron create -f job_spec.json
```
...
- 缺少参数时，询问用户补充后再创建
- 暂停/删除/恢复前，用 `hidata cron list` 查找 job_id
- 排查问题时，用 `hidata cron state <job_id>` 查看状态
- 给用户的命令要完整、可直接复制执行
