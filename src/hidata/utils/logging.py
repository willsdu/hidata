# -*- coding: utf-8 -*-
"""Logging setup for HiData: console output and optional file handler."""
import logging
...
_HIDATA_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_HIDATA_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (hidata), not deps."""
    ...
    # Only attach handler to our namespace so only hidata.* logs are printed.
...
def add_hidata_file_handler(log_path: Path) -> None:
    """Add a file handler to the hidata logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "hidata.log").
    """
    ...
    if is_windows_or_linux:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path,
            encoding="utf-8",
            maxBytes=_HIDATA_LOG_MAX_BYTES,
            backupCount=_HIDATA_LOG_BACKUP_COUNT,
        )
